...
確認用の起動：環境変数 `AMIZ_SCREEN` に `sample`（くまの頭）／`motif`（花のモチーフ）／`rabbit`（うさぎの胴体・螺旋編み）／`big`（40段）／`symbols`（記号一覧）を渡すと、保存しないサンプルでその画面が開く（`SIMCTL_CHILD_AMIZ_SCREEN=sample xcrun simctl launch <UDID> com.akira.Amiz`、または Xcode のスキームの環境変数）。
