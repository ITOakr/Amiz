...
- タスク管理は Jira で行う。チケットの作成・更新も Claude Code が行う
- 作業開始時にチケットを In Progress にする
- ブランチ名とコミットメッセージの先頭にチケットキーを入れる（例：`CROCHET-12 目のデータ構造を追加`）
...
- 完了時に、変更の概要と確認方法をチケットにコメントしてから Done にする
- チケットは削除しない。不要になったものはクローズする
